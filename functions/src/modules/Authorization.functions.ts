...
export const onCreateCustomClaimRoles = userRoles
...
export const onDeleteCustomClaimRoles = userRoles
...
export const onCreateUserCustomClaimRoles = auth.user()
    .onCreate(async (event) => {
      ...
      if (userData && userData.roles) {
        return admin.auth().setCustomUserClaims(event.uid, userData.roles).catch(() => null);
      }
      return null;
    });
